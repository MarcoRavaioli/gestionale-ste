...
export class NuovaFatturaModalComponent implements OnInit {
  ...
  constructor(
    ...
    addIcons({ cloudUploadOutline, close, attachOutline, documentTextOutline, walletOutline, businessOutline, calendarOutline, personAddOutline });
    ...
    this.form = this.fb.group({
      ...
      data_scadenza: [this.calcolaScadenza(new Date().toISOString()), Validators.required],
  ...
  ngOnInit() {
    ...
    this.form.get('data_emissione')?.valueChanges.subscribe(dateIso => {
      if (dateIso) {
        ...
        this.form.patchValue({ data_scadenza: nuovaScadenza }, { emitEvent: false });
      }
    });
  ...
  salva() {
    ...
    this.fatturaService.createWithAttachment(rawData, this.fileSelezionato).subscribe({
      next: (res) => {
