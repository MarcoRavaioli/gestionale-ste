...
export class NuovoAppuntamentoGlobaleModalComponent implements OnInit {
  ...
  nuovaCommessa = { seriale: '', descrizione: '', stato: 'APERTA', valore_totale: null };
  // Aggiunti campi mancanti per coerenza
  nuovoCantiere = { via: '', civico: '', citta: '', cap: '', provincia: '', stato: 'Italia' };
  ...
  constructor(
    ...
    addIcons({ calendarOutline, documentsOutline, locationOutline, personAddOutline, closeOutline, searchOutline, add });
  }

  ngOnInit() {
    // Caricamento parallelo dati
    this.cliService.getAll().subscribe(d => this.listaClienti = d.sort((a,b) => a.nome.localeCompare(b.nome)));
    this.indService.getAll().subscribe(d => this.listaCantieri = d.sort((a,b) => a.citta.localeCompare(b.citta)));
    this.comService.getAll().subscribe(d => this.listaCommesse = d.sort((a,b) => a.seriale.localeCompare(b.seriale)));
  }
  ...
  toggleModeCommessa() { 
    this.modeCommessa = this.modeCommessa === 'esistente' ? 'nuova' : 'esistente'; 
  ...
  async salva() {
      ...
      if (this.modeCommessa === 'esistente') {
        ...
        if (this.modeCantiere === 'esistente') {
          ...
          if (this.modeCliente === 'esistente') {
            ...
            // CREA NUOVO CLIENTE
            const cli = await this.wrap(this.cliService.create(this.nuovoCliente));
            clienteIdFinale = cli.id;
          }

          // Crea Cantiere
          const indPayload = { ...this.nuovoCantiere, cliente: { id: clienteIdFinale } } as any;
          const ind = await this.wrap(this.indService.create(indPayload));
          indirizzoIdFinale = ind.id;
        }

        // Crea Commessa
        const comPayload = { ...this.nuovaCommessa, indirizzo: { id: indirizzoIdFinale } } as any;
    ...
    } catch (err) { console.error('Errore cascata', err); }
  }

  // Helper per trasformare Observable in Promise
  wrap(obs: any): Promise<any> {
    return new Promise((resolve, reject) => {
      obs.subscribe({ next: (res: any) => resolve(res), error: (err: any) => reject(err) });
    });
  }
