  ...
  getLabelValue(): string {
    ...
    switch (this.type) {
      case 'cliente':
        return item.nome;
      case 'cantiere':
        return `${item.citta} - ${item.via}`;
      case 'commessa':
        return `${item.seriale} - ${item.descrizione || '...'}`;
      default:
        return 'Selezionato';
    }
  }
    ...
    this.filteredItems = this.items.filter((item) => {
      switch (this.type) {
        ...
        case 'cantiere':
          ...
          ); // Cerca anche per cliente proprietario!

        case 'commessa':
          return (
            item.seriale.toLowerCase().includes(term) ||
            item.descrizione?.toLowerCase().includes(term) ||
            item.indirizzo?.cliente?.nome.toLowerCase().includes(term)
          ); // Cerca anche per cliente
